<?php

// This file is auto-generated, don't edit it. Thanks.
namespace AlibabaCloud\SDK\SDK_DATA_1598238000612\Models;

use AlibabaCloud\Tea\Model;

class CreateOapiDepartmentParams extends Model {
    protected $_name = [
        'parentid' => 'parentid',
        'parentBalanceFirst' => 'parentBalanceFirst',
        'shareBalance' => 'shareBalance',
        'outerPermitUsers' => 'outerPermitUsers',
        'outerPermitDepts' => 'outerPermitDepts',
        'outerDept' => 'outerDept',
        'userPerimits' => 'userPerimits',
        'deptPerimits' => 'deptPerimits',
        'deptHiding' => 'deptHiding',
        'createDeptGroup' => 'createDeptGroup',
        'order' => 'order',
        'name' => 'name',
        'sourceIdentifier' => 'sourceIdentifier',
        'deptPermits' => 'deptPermits',
        'userPermits' => 'userPermits',
        'outerDeptOnlySelf' => 'outerDeptOnlySelf',
    ];
    public function validate() {}
    public function toMap() {
        $res = [];
        if (null !== $this->parentid) {
            $res['parentid'] = $this->parentid;
        }
        if (null !== $this->parentBalanceFirst) {
            $res['parentBalanceFirst'] = $this->parentBalanceFirst;
        }
        if (null !== $this->shareBalance) {
            $res['shareBalance'] = $this->shareBalance;
        }
        if (null !== $this->outerPermitUsers) {
            $res['outerPermitUsers'] = $this->outerPermitUsers;
        }
        if (null !== $this->outerPermitDepts) {
            $res['outerPermitDepts'] = $this->outerPermitDepts;
        }
        if (null !== $this->outerDept) {
            $res['outerDept'] = $this->outerDept;
        }
        if (null !== $this->userPerimits) {
            $res['userPerimits'] = $this->userPerimits;
        }
        if (null !== $this->deptPerimits) {
            $res['deptPerimits'] = $this->deptPerimits;
        }
        if (null !== $this->deptHiding) {
            $res['deptHiding'] = $this->deptHiding;
        }
        if (null !== $this->createDeptGroup) {
            $res['createDeptGroup'] = $this->createDeptGroup;
        }
        if (null !== $this->order) {
            $res['order'] = $this->order;
        }
        if (null !== $this->name) {
            $res['name'] = $this->name;
        }
        if (null !== $this->sourceIdentifier) {
            $res['sourceIdentifier'] = $this->sourceIdentifier;
        }
        if (null !== $this->deptPermits) {
            $res['deptPermits'] = $this->deptPermits;
        }
        if (null !== $this->userPermits) {
            $res['userPermits'] = $this->userPermits;
        }
        if (null !== $this->outerDeptOnlySelf) {
            $res['outerDeptOnlySelf'] = $this->outerDeptOnlySelf;
        }
        return $res;
    }
    /**
     * @param array $map
     * @return CreateOapiDepartmentParams
     */
    public static function fromMap($map = []) {
        $model = new self();
        if(isset($map['parentid'])){
            $model->parentid = $map['parentid'];
        }
        if(isset($map['parentBalanceFirst'])){
            $model->parentBalanceFirst = $map['parentBalanceFirst'];
        }
        if(isset($map['shareBalance'])){
            $model->shareBalance = $map['shareBalance'];
        }
        if(isset($map['outerPermitUsers'])){
            $model->outerPermitUsers = $map['outerPermitUsers'];
        }
        if(isset($map['outerPermitDepts'])){
            $model->outerPermitDepts = $map['outerPermitDepts'];
        }
        if(isset($map['outerDept'])){
            $model->outerDept = $map['outerDept'];
        }
        if(isset($map['userPerimits'])){
            $model->userPerimits = $map['userPerimits'];
        }
        if(isset($map['deptPerimits'])){
            $model->deptPerimits = $map['deptPerimits'];
        }
        if(isset($map['deptHiding'])){
            $model->deptHiding = $map['deptHiding'];
        }
        if(isset($map['createDeptGroup'])){
            $model->createDeptGroup = $map['createDeptGroup'];
        }
        if(isset($map['order'])){
            $model->order = $map['order'];
        }
        if(isset($map['name'])){
            $model->name = $map['name'];
        }
        if(isset($map['sourceIdentifier'])){
            $model->sourceIdentifier = $map['sourceIdentifier'];
        }
        if(isset($map['deptPermits'])){
            $model->deptPermits = $map['deptPermits'];
        }
        if(isset($map['userPermits'])){
            $model->userPermits = $map['userPermits'];
        }
        if(isset($map['outerDeptOnlySelf'])){
            $model->outerDeptOnlySelf = $map['outerDeptOnlySelf'];
        }
        return $model;
    }
    /**
     * @description 父部门id。根部门id为1
     * @var string
     */
    public $parentid;

    /**
     * @description 是否优先使用父部门的预算
     * @var bool
     */
    public $parentBalanceFirst;

    /**
     * @description 是否共享预算
     * @var bool
     */
    public $shareBalance;

    /**
     * @description 本部门的员工仅可见员工自己为true时，可以配置额外可见人员，值为userid组成的的字符串，使用|符号进行分割。总数不能超过200。
     * @var string
     */
    public $outerPermitUsers;

    /**
     * @description 本部门的员工仅可见员工自己为true时，可以配置额外可见部门，值为部门id组成的的字符串，使用|符号进行分割。总数不能超过200。
     * @var string
     */
    public $outerPermitDepts;

    /**
     * @description 是否本部门的员工仅可见员工自己, 为true时，本部门员工默认只能看到员工自己
     * @var bool
     */
    public $outerDept;

    /**
     * @description 已废弃
     * @var string
     */
    public $userPerimits;

    /**
     * @description 已废弃
     * @var string
     */
    public $deptPerimits;

    /**
     * @description 是否隐藏部门, true表示隐藏, false表示显示
     * @var bool
     */
    public $deptHiding;

    /**
     * @description 是否创建一个关联此部门的企业群，默认为false
     * @var bool
     */
    public $createDeptGroup;

    /**
     * @description 在父部门中的次序值。order值小的排序靠前
     * @var string
     */
    public $order;

    /**
     * @description 部门名称。长度限制为1~64个字符。不允许包含字符‘-’‘，’以及‘,’
     * @var string
     */
    public $name;

    /**
     * @description 部门标识字段，开发者可用该字段来唯一标识一个部门，并与钉钉外部通讯录里的部门做映射
     * @var string
     */
    public $sourceIdentifier;

    /**
     * @description 可以查看指定隐藏部门的其他部门列表，如果部门隐藏，则此值生效，取值为其他的部门id组成的的字符串，使用 | 符号进行分割。总数不能超过200。
     * @var string
     */
    public $deptPermits;

    /**
     * @description 可以查看指定隐藏部门的其他人员列表，如果部门隐藏，则此值生效，取值为其他的人员userid组成的的字符串，使用| 符号进行分割。总数不能超过200。
     * @var string
     */
    public $userPermits;

    /**
     * @description 是否只能看到所在部门及下级部门通讯录
     * @var bool
     */
    public $outerDeptOnlySelf;

}
